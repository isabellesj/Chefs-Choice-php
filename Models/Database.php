...
class DBContext
{
    ...
    function searchProducts($sortCol, $sortOrder, $q, $categoryId) //$pageNo = 1, $pageSize = 20
    {
        ...
        if ($categoryId != null && strlen($categoryId) > 0) {
            ...
            $sql = $sql . " ( CategoryId = :categoryId )"; //categoryId = :categoryId?
            $paramsArray["categoryId"] = $categoryId;
            //vad är categoryid och officeid i det här fallet??
        }
...
// {
// return $this->pdo->query('select * from Customer order by Popularity desc limit 0,10')->fetchAll(PDO::FETCH_CLASS,
// 'Customer');

// } //hämtar populäraste kunderna (i vårt fall produkter)
